...
///  3. When the pool's real ETH reserve reaches `GRAD_ETH`, the pool graduates automatically:
///     ETH + spot-price-matched tokens are deposited into a Uniswap v2 pair, the LP tokens are
///     sent to the dead address (liquidity locked forever), the remaining curve tokens are
///     burned, and curve trading closes.
///
...
contract CurveLaunchpad {
    ...
    function buy(address token, uint256 minTokensOut) external payable nonReentrant returns (uint256 tokensOut) {
        ...
        if (pool.realEth >= GRAD_ETH) _graduate(token, pool);
    }
    ...
    /// @dev Deposits the pool's ETH plus spot-price-matched tokens into a fresh v2 pair, sends
    ///      the LP tokens to the dead address, and burns every remaining curve token so the DEX
    ///      opens at exactly the curve's closing price with no overhang.
    function _graduate(address token, Pool storage pool) private {
        ...
        // amountMins are 0: the pair is expected to be fresh; if someone pre-seeded it to skew
        // the ratio, the router refunds the unused side rather than bricking graduation.
        // Flagged for the pre-mainnet audit.
        ROUTER.addLiquidityETH{value: ethLp}(token, tokensLp, 0, 0, DEAD, block.timestamp);
